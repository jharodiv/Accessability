...
class _SafetyAssistWidgetState extends State<SafetyAssistWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return DraggableScrollableSheet(
      ...
      expand: true, // allow full-screen expansion
      initialChildSize: _showHelper ? 0.8 : 0.5,
      minChildSize: 0.5,
      maxChildSize: 1.0, // <- allow dragging to utmost top
      builder: (BuildContext context, ScrollController scrollController) {
        // NOTE: ServiceButtons placed OUTSIDE the main white container (above it),
        // and wrapped with IgnorePointer + AnimatedOpacity so they fade/disable when expanded/helper shown.
        return Column(
          children: [
            const SizedBox(height: 8),

            // OUTSIDE ServiceButtons
            IgnorePointer(
            ...
            // MAIN container (white/dark) - same contents as before
            Expanded(
              child: SafeArea(
                top:
                    _isExpanded, // only apply top safe inset when sheet is expanded
                bottom: false, // keep bottom behavior unchanged
                child: Container(
                  decoration: BoxDecoration(
